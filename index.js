...
*/
function getCarInfoByIndex(carInfoArray, targetid) {
  return "This is a " + carInfoArray[targetid].car_make + " " + carInfoArray[targetid].car_model;
}
...
*/
function getLastCarInfo(carInfoArray) {
  return "This is a " + carInfoArray[carInfoArray.length -1].car_make + " " + carInfoArray[carInfoArray.length -1].car_model
}
 ...
 * it will return `This is a Lincoln Navigator`.
*/
function getCarInfoById(carInfoArray, targetid) {
  for (let i = 0; i < carInfoArray.length; i++) {
    if (carInfoArray[i].id = targetid) {
      return "This is a " + carInfoArray[i].car_make + " " + carInfoArray[i].car_model;
    }
  }
}
...
*/
function sortCarInventory(carInfoArray) {
  return carInfoArray.sort(function(carData1, carData2){
     const dif = carData1.car_model.localeCompare(carData2.car_model);
     return dif;
  });
...
*/
function getModelYears(carInfoArray) {
 return carInfoArray.map(function(item){
...
*/
function getOlderCars(carInfoArray, maxYear) {
  const carResult = [];
  for (let i = 0; i < carInfoArray.length; i++){
    if (carInfoArray[i].car_year <= maxYear)
    carResult.push(carInfoArray[i])
  }
  return carResult;
}
 ...
 * in the same order as they appear in the original inventory.
*/
function getGermanCars(/**carInfoArray*/) {
